...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier isOwner() {
    require(msg.sender == owner, "Something Wrong");
    _;
  }
  modifier verifyCaller(address _address) { 
    require (msg.sender == _address, "Address Not Verify");
    _;
  }

  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "Not Enough Coin");
     _;
  }
  ...
  modifier forSale(uint sku) {
    require(items[sku].state == State.ForSale, "State != ForSale");
    _;
  }
  modifier sold(uint sku) {
    require(items[sku].state == State.Sold, "State != Sold");
    _;
  }
  modifier shipped(uint sku) {
    require(items[sku].state == State.Shipped, "Sate != Shipped");
    _;
  }
  modifier received(uint sku) {
    require(items[sku].state == State.Received, "State != Received");
    _;
  }
  ...
  function buyItem(uint sku) public payable checkValue(sku) forSale(sku) paidEnough(msg.value)
  {
